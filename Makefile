...
# Render source files
RENDER_SRCS = $(RENDER_DIR)/render.c $(RENDER_DIR)/screenshot.c $(RENDER_DIR)/sprites.c $(RENDER_DIR)/keymap.c $(RENDER_DIR)/config.c
...
$(BUILD_DIR)/keymap.o: $(RENDER_DIR)/keymap.c
	$(CC) $(CFLAGS) $(SDL_CFLAGS) -I$(SRC_DIR) -c $< -o $@

$(BUILD_DIR)/config.o: $(RENDER_DIR)/config.c
	$(CC) $(CFLAGS) $(SDL_CFLAGS) -I$(SRC_DIR) -c $< -o $@

$(BUILD_DIR)/main_render.o: $(RENDER_DIR)/main.c
	$(CC) $(CFLAGS) $(SDL_CFLAGS) -I$(SRC_DIR) -c $< -o $@

$(RENDER_BIN): $(OBJS) $(BUILD_DIR)/render.o $(BUILD_DIR)/screenshot.o $(BUILD_DIR)/sprites.o $(BUILD_DIR)/keymap.o $(BUILD_DIR)/config.o $(BUILD_DIR)/main_render.o
